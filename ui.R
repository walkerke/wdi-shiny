...
shinyUI(
  fluidPage(
    ...
    tabsetPanel(
      tabPanel("Controls", 
               selectInput('indicator', 'World Development Indicator', lookup_list2, 
      ...
      tabPanel("About", 
              ...
              p('Select a World Development Indicator to query the World Bank API and return an interactive
                Leaflet map.  The map defaults to Total Fertility Rate in 2012. To find the indicator you want, 
                type a term into the "World Development Indicator" search box and the app will give you suggestions.
                Then, click any country to produce an interactive dygraphs time-series chart, available
                from the "Charts" tab (the charts work best in Firefox at the moment).' ), 
              p('Please note that indicators are often not available for all countries for all years.  Also: this
